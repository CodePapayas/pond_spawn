...
impl ClusterState {
    ...
    pub fn run(genomes: &[Genome], k_genome: usize, k_brain: usize, step: u32, prev: Option<&ClusterState>) -> Self {
        let n = genomes.len();
        if n == 0 {
            return Self::new();
        }
        ...
        Self { genome_cluster_ids, brain_cluster_ids, genome_centroids }
    }
}
...
mod tests {
    ...
    fn cluster_output_length_matches_input() {
        ...
        let state = ClusterState::run(&genomes, 6, 8, 100, None);
    ...
    fn cluster_ids_in_range() {
        ...
        let state = ClusterState::run(&genomes, 4, 6, 50, None);
    ...
    fn cluster_handles_small_population() {
        ...
        let state = ClusterState::run(&genomes, 6, 16, 50, None);
    ...
    fn cluster_empty_population() {
        let state = ClusterState::run(&[], 6, 24, 50, None);
    ...
    fn labels_stable_across_reruns_with_prev() {
        ...
        let a = ClusterState::run(&genomes, 6, 8, 100, None);
        let b = ClusterState::run(&genomes, 6, 8, 150, Some(&a));
    ...
    fn cluster_deterministic() {
        ...
        let a = ClusterState::run(&genomes, 4, 8, 200, None);
        let b = ClusterState::run(&genomes, 4, 8, 200, None);
        assert_eq!(a.genome_cluster_ids, b.genome_cluster_ids);
