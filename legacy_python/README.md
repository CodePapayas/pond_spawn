# Legacy Python Sim (archived)

This directory holds the **original Python + PyTorch implementation** of Pond
Spawn. It is preserved for reference only — the canonical, actively developed
engine is now [`pond_core`](../pond_core/README.md) (Rust/WASM), driven by the
[`pond_web`](../pond_web/) renderer. See [`../README.md`](../README.md) for
the current project overview and [`../REFACTOR_RUST_ROADMAP.md`](../REFACTOR_RUST_ROADMAP.md)
for the refactor log.

**This code is frozen.** No further feature work, balance changes, or CI runs
against it — it is not linted or tested in CI anymore. Test suite (`tests/`)
still exists and passes as of the archive date if you want to run it locally.

All commands below run from inside `legacy_python/`.
